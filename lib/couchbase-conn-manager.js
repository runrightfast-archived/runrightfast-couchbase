...
(function() {
	...
	/**
	 * 
	 * @returns number of connections that were registered
	 */
	ConnectionManager.prototype.getConnectionCount = function() {
		return this._conns.length;
	};

	/**
	 * 
	 * @returns number of bucket connections that were registered
	 */
	ConnectionManager.prototype.getBucketCount = function() {
	 ...
	 *   	]
	 *   },
	 *   start : true										// OPTIONAL - if true, then starts the connection after it is registered. Default is true
	 *   startCallback : function(result){} 				// OPTIONAL - result is either an Error. This only applies if start = true.
	...
	ConnectionManager.prototype.registerConnection = function(options) {
		...
		var validateOptions = function() {
			...
			if (!lodash.isUndefined(options.start)) {
				assert(lodash.isBoolean(options.start), 'options.start is required and must be a Boolean');
			}

			if (!lodash.isUndefined(options.startCallback)) {
				assert(lodash.isFunction(options.startCallback), 'options.startCallback is required and must be a Function');
			}
		};
		...
		for (i = 0; i < options.couchbase.buckets.length; i++) {
			...
			if (options.start) {
				conn.start(options.startCallback);
			}
